...
# filtrer les répertoires dont le nom commence par ETAB_FOLDER
...
for d in diplomes:
    ...
    if TEMPLATE_CANDIDAT_SHEET not in wb.sheetnames:
        print(f"❌ Le fichier \"{classeur}\" doit posséder une feuille \"{TEMPLATE_CANDIDAT_SHEET}\" !\n")
        sys.exit(8)
    wb.close()
...
# vérification : la feuille TEMPLATE_CANDIDAT_SHEET existe dans le fichier TEMPLATE_FILE
wb = openpyxl.load_workbook(TEMPLATE_FILE, read_only=True, data_only=True)
if TEMPLATE_CANDIDAT_SHEET not in wb.sheetnames:
    print(f"❌ Le fichier \"{TEMPLATE_FILE}\" doit posséder une feuille \"{TEMPLATE_CANDIDAT_SHEET}\" !\n")
    sys.exit()
wb.close()
